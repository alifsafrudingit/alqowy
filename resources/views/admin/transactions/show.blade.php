    ...
    <x-slot name="header">
        <div class="flex flex-row justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                 <a href="{{ route('admin.subscribe_transactions.index') }}" onclick="windiw.history.go(-1); return false">
            {{ __('Subscriptions') }}
          </a>
          {!! __('&raquo; Detail Information ') !!}
            </h2>
        </div>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-10 flex flex-col gap-y-5">
                <div class="flex flex-row gap-x-10">
                    ...
                    <div class="flex flex-col gap-y-10">    
                        ...
                        <div>
                            ...
                            <h3 class="text-indigo-950 text-xl font-bold">{{ $subscribeTransaction->created_at->isoFormat('ddd, DD MMMM Y') }}</h3>
                            <p class="text-slate-500 text-sm">{{ $subscribeTransaction->created_at->diffForHumans() }}</p>
                        </div>
                        <div>
                          <p class="text-slate-500 text-sm">Subscription Start At</p>
                          @if ($subscribeTransaction->subscription_start_date)
                          <h3 class="text-indigo-950 text-xl font-bold">{{ date('D, d M Y', strtotime($subscribeTransaction->subscription_start_date)) }}</h3>
                          @else
                    ...
                    <div>
                        ...
                        @else
                        <img src="{{ Storage::url('proof/no-image-payment.jpg') }}" alt="" class="w-60">
                ...
                @if ($subscribeTransaction->subscription_start_date)
                <hr class="my-5">
                <form action="{{ route('admin.subscribe_transactions.index') }}">
                <button type="submit" class="font-bold py-4 px-6 bg-orange-400 text-white rounded-full">
                  Back
                </button>
                @else
                <hr class="my-5">
